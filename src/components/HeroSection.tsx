...
export default function HeroSection() {
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden px-6 rangoli-bg"
      style={{ background: 'linear-gradient(180deg, #FFF8E7 0%, #FFF5D6 40%, #FFF8E7 100%)' }}>

      {/* Mandala watermark */}
      <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
        <div className="w-[600px] h-[600px] md:w-[800px] md:h-[800px] rounded-full opacity-[0.03]"
          style={{
            border: '2px solid #D4AF37',
            background: 'radial-gradient(circle, transparent 30%, transparent 50%)',
            boxShadow: 'inset 0 0 100px rgba(212,175,55,0.05), 0 0 0 40px rgba(212,175,55,0.02), 0 0 0 80px rgba(212,175,55,0.01)',
          }}
        />
      </div>
      ...
      <div className="absolute inset-0 pointer-events-none"
        style={{ background: 'radial-gradient(ellipse at 50% 40%, rgba(212,175,55,0.08) 0%, transparent 50%)' }} />
      ...
      <motion.div variants={stagger} initial="hidden" animate="visible" className="text-center z-10 max-w-3xl mx-auto">
        {/* Ganesh blessing */}
        ...
        <motion.h1 variants={fadeUp} className="leading-tight mb-4 px-2 overflow-visible">
          <span className={`block ${isTe ? 'font-telugu text-4xl sm:text-5xl md:text-6xl font-bold' : 'font-display text-5xl sm:text-6xl md:text-8xl'} gold-gradient-text`}>
            {isTe ? 'త్రయ' : 'Thraya'}
          </span>
          <span className="block font-script text-2xl md:text-4xl my-2 md:my-3" style={{ color: '#D4AF37' }}>&amp;</span>
          <span className={`block ${isTe ? 'font-telugu text-4xl sm:text-5xl md:text-6xl font-bold' : 'font-display text-5xl sm:text-6xl md:text-8xl'} gold-gradient-text`}>
            {isTe ? 'భరత్' : 'Bharath'}
          </span>
        </motion.h1>
        ...
        <motion.div variants={fadeUp} className="flex items-center justify-center gap-3 my-6 md:my-8">
          ...
          <svg width="24" height="24" viewBox="0 0 24 24" fill="#D4AF37" opacity="0.7">
      ...
      <motion.div animate={{ y: [0, 8, 0] }} transition={{ duration: 2.5, repeat: Infinity }}
        className="absolute bottom-6 left-1/2 -translate-x-1/2 text-center" style={{ color: '#D4AF37' }}>
